...
def get_lr(it):
    if it < warump_steps:
        return max_lr*((it+1)/warump_steps)
    if it > max_steps:
        return min_lr
    decay_ratio = (it-warump_steps)/(max_steps-warump_steps)
...
for i in range(50):
    ...
    print(
        f'step{i}, loss {loss.item()}, time {(t1-t0)*1000:.4f}ms, tok/sec:{tokens_per_second:.2f}')

# ----------------------------------#
# using weights from huggingface
